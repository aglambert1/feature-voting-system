...
const Stage5_Finalization = ({
  ...
  const [_submittedIdeaIds, setSubmittedIdeaIds] = useState<number[]>([]);

  useEffect(() => {
    loadApprovedIdeas();
  }, [sessionId]);

  const loadApprovedIdeas = async (): Promise<void> => {
    setLoading(true);
    try {
      const response = await api.get<{ ideas: ApprovedIdea[] }>(
        `/product-intelligence/sessions/${sessionId}/generated-ideas`
      );
      const allIdeas = response.data.ideas || [];
      const approved = allIdeas.filter((idea) => (idea as any).user_approved);
